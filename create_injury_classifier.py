...
def main():
    ...
    log_reg, svr_reg, knn_reg, mlp_reg = train_clf(X_1, X_2, Y_1, Y_2, columns)
    dump_regs(log_reg, svr_reg, knn_reg, mlp_reg)
    return
...
def train_clf(X_1, X_2, Y_1, Y_2, columns):                       
    ...
    #output = dnn_clf.predict(X_2)
    #print_table()
    #pos = input("Enter the position: [0-9] ")
    #age = input("Enter the age: ")
    #height = input("Enter the height: ")
    #weight = input("Enter the weight: ")
    #team = input("Enter the team played on: [1 - 33] ")
    #against = input("Enter the opposing team: [0 - 33] ")
    #home_away = input("Enter the whether the player played on home[0] or away[1]: ")
    #concussion = input("Enter if player has had a concussion previously: No [0] Yes [1]")
    
    #inputted_x = [pos, age, height, weight, team, home_away, against, concussion]
    
    #outputted_y = dnn_clf.predict(inputted_x)
    #print(outputted_y)
    
    #print(output)
    #output = knn_reg.predict(X_2)
    #output = svr_reg.predict(X_2)

    #from sklearn.metrics import confusion_matrix
    #matrix = confusion_matrix(output, Y_2)
    #print (matrix)
    
    return log_reg, svr_reg, knn_reg, mlp_reg
###############################################################################
#
...
#
def dump_regs(log_reg, svr_reg, knn_reg, mlp_reg):
    from sklearn.externals import joblib
    #joblib.dump(log_reg, '/log_reg_model_nb.pkl')
    #joblib.dump(knn_reg, '/knn_reg_model_nb.pkl')
    #joblib.dump(svr_reg, '/svr_reg_model_nb.pkl')
    #joblib.dump(mlp_reg, '/mlp_reg_model_nb.pkl')
    return
